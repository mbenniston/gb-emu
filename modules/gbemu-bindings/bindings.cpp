...
DebugInstruction::~DebugInstruction() { delete[] mnemonic; }
...
int Emulator::debug_tick(JoypadState *joypadState) {
  ...
  int i = 0;
  for (;;) {
    if (i > 0 && std::find(breakpoints.begin(), breakpoints.end(),
                           emulator.cpu.registers.PC) != breakpoints.end()) {
      hit_breakpoint = true;
      break;
    }
    int cycles_elapsed;
...
void *Emulator::get_framebuffer_ptr() { return emulator.lcd.framebuffer; }

int Emulator::get_framebuffer_width() { return FB_WIDTH; }

int Emulator::get_framebuffer_height() { return FB_HEIGHT; }

int Emulator::get_framebuffer_channels() { return FB_CHANNELS; }
...
DebugTimerControllerState *Emulator::get_debug_timer_controller_state() {
  ...
  state->tac_enabled =
      (emulator.timer_controller.register_tac & TAC_ENABLED_BIT) != 0;
...
DebugInterruptsState *Emulator::get_debug_interrupts_state() {
  ...
  state->joypad_requested =
      emulator.interrupt_controller.register_if & IF_JOYPAD;
  state->serial_requested =
      emulator.interrupt_controller.register_if & IF_SERIAL;
  ...
  state->vblank_requested =
      emulator.interrupt_controller.register_if & IF_VBLANK;
...
DebugInstructionTable *Emulator::get_debug_instruction_table() {
  ...
  for (int i = 0; i < 256; i++) {
    ...
    strncpy(table->extended_instructions[i]->mnemonic, instruction.name,
            name_length);
...
int Emulator::get_debug_memory_max() { return BUS_MAX_ADDRESS; }

DebugMemoryDump *Emulator::get_debug_dump_memory_range(long range_start,
                                                       long range_length) {
    ...
    for (int j = 0; j < NUM_TILES_Y; j++) {
      ...
      for (int y = 0; y < 8; y++) {
        for (int x = 0; x < 8; x++) {
          ...
          int low_value =
              (emulator.ppu.vram[row_index - VRAM_ADDRESS] >> (7 - x)) & 0b1;
          int high_value =
              (emulator.ppu.vram[row_index + 1 - VRAM_ADDRESS] >> (7 - x)) &
              0b1;
...
DebugTileSetDump::~DebugTileSetDump() { delete[] data; }
