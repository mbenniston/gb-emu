...
void joypad_register_write(joypad_t *joypad, uint8_t data) {
  ...
  if (buttons) {
    ...
    joypad->register_joyp |=
        !joypad->state.start_pressed ? JOYPAD_START_DOWN : 0;
    joypad->register_joyp |=
        !joypad->state.select_pressed ? JOYPAD_SELECT_UP : 0;
  ...
  if (dpad) {
    joypad->register_joyp &= 0xF0;
    joypad->register_joyp |=
        !joypad->state.down_pressed ? JOYPAD_START_DOWN : 0;
