...
const char *cartidge_title(cartridge_t *rom) { return rom->title; }

cartridge_type_e cartridge_type(cartridge_t *rom) { return rom->type; }
...
int cartridge_load(cartridge_t *rom, uint8_t *data, uint32_t data_length) {
  ...
  rom->type = data[CARTRIDGE_TYPE_ADDRESS];

  if (!mbc_create(&rom->mbc, rom->type, data, data_length)) {
    return 0;
  }
...
void cartridge_destroy(cartridge_t *rom) { mbc_destroy(&rom->mbc); }
