...
void timer_controller_tick(timer_controller_t *controller, int machine_cycles_elapsed, int *timer_interrupt_requested) {
    ...
    if (controller->machine_cycles_since_last_increment > increment) {
        amount_to_increment_tima = controller->machine_cycles_since_last_increment / increment;
        uint16_t left_over_cycles = controller->machine_cycles_since_last_increment % increment;
        controller->machine_cycles_since_last_increment = left_over_cycles;
    }

    if (controller->register_tac & TAC_ENABLED_BIT) {
        int tima_will_overflow = controller->register_tima + amount_to_increment_tima > 0xFF;
