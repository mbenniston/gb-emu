...
void bus_create(bus_t *bus, interrupt_controller_t *interrupt_controller, timer_controller_t *timer_controller,
                lcd_t *lcd, ppu_t *ppu, joypad_t *joypad, cartridge_t *cartridge) {
...
void bus_write(bus_t *bus, uint16_t address, uint8_t data) {
    ...
    if (address >= CARTRIDGE_RAM_START_ADDRESS && address <= CARTRIDGE_RAM_END_ADDRESS) {
        cartridge_write(bus->cartridge, address, data);
        return;
    }
    ...
    // echo work ram
    if (address >= ECHO_WORK_RAM_START_ADDRESS && address <= ECHO_WORK_RAM_END_ADDRESS) {
        bus->memory[WORK_RAM_START_ADDRESS + address - ECHO_WORK_RAM_START_ADDRESS] = data;
        return;
    }
...
uint8_t bus_read(bus_t *bus, uint16_t address) {
    // cartridge
    if (address <= CARTRIDGE_ROM_END_ADDRESS ||
        (address >= CARTRIDGE_RAM_START_ADDRESS && address <= CARTRIDGE_RAM_END_ADDRESS)) {
    ...
    if (address >= VRAM_ADDRESS && address < VRAM_ADDRESS + VRAM_LENGTH && address <= 0x9FFF) {
        return bus->ppu->vram[address - VRAM_ADDRESS];
    }
    if (address >= OAM_ADDRESS && address < OAM_ADDRESS + OAM_LENGTH && address <= 0xFE9F) {
    ...
    // echo work ram
    if (address >= ECHO_WORK_RAM_START_ADDRESS && address <= ECHO_WORK_RAM_END_ADDRESS) {
        return bus->memory[WORK_RAM_START_ADDRESS + address - ECHO_WORK_RAM_START_ADDRESS];
    }
