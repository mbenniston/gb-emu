...
uint8_t mbc_mbc3_ram_battery_read(mbc_mbc3_ram_battery_t *mbc, uint16_t address) {
    // bank 0
    if (address <= 0x3FFF) return mbc->rom_data[address];

    // ram
    if (address >= 0xA000 && address <= 0xBFFF) {
        if (!mbc->ram_and_timer_enabled) return 0xFF;

        return mbc->ram_data[
            mbc->ram_bank_number * 8 * 1024 + address - 0xA000];
    }
...
void mbc_mbc3_ram_battery_write(mbc_mbc3_ram_battery_t *mbc, uint16_t address, uint8_t data) {
    ...
    if (address >= RAM_BANK_NUMBER_START_WRITE_ADDRESS && address <= RAM_BANK_NUMBER_END_WRITE_ADDRESS) {
        mbc->ram_bank_number = data & 0b11;
        return;
    }

    if (address >= ROM_BANK_NUMBER_START_ADDRESS && address <= ROM_BANK_NUMBER_END_ADDRESS) {
    ...
    if (address >= 0xA000 && address <= 0xBFFF) {
        if (mbc->ram_and_timer_enabled) {
            mbc->ram_data[mbc->ram_bank_number * 8 * 1024 + (address - 0xA000)] = data;
        }
    }
}
