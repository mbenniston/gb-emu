...
void get_background(ppu_t *ppu, lcd_t *lcd, int screen_x, int screen_y, int *palette_index, int *color) {
    // apply scroll,
    int tile_x = ((screen_x + ppu->register_scx) % 256) / 8;
    int tile_y = ((screen_y + ppu->register_scy) % 256) / 8;

    int tile_map_address = ((lcd->register_lcdc & LCDC_BG_TILE_MAP) > 0 ? 0x9C00 : 0x9800) - VRAM_ADDRESS;
    ...
    if ((lcd->register_lcdc & LCDC_BG_AND_WINDOW_TILES) == 0) {
        tile_start = (tile < 128 ? tile * 16 + 0x9000 : ((tile - 128) * 16 + 0x8800));
    }
    ...
    int low_value = (ppu->vram[row_index - VRAM_ADDRESS] >> (7 - tile_pixel_x)) & 0b1;
    int high_value = (ppu->vram[row_index + 1 - VRAM_ADDRESS] >> (7 - tile_pixel_x)) & 0b1;
    *palette_index = (low_value | high_value << 1) & 0b11;

    *color = (ppu->register_bgp & (0b11 << *palette_index * 2)) >> *palette_index * 2;
}

void get_window(ppu_t *ppu, lcd_t *lcd, int screen_x, int screen_y, int *palette_index, int *color) {
    // apply scroll,
    int tile_x = ((screen_x) % 256) / 8;
    int tile_y = ((screen_y) % 256) / 8;

    int tile_map_address = ((lcd->register_lcdc & LCDC_WINDOW_TILE_MAP) > 0 ? 0x9C00 : 0x9800) - VRAM_ADDRESS;
    uint16_t tile_address = tile_map_address + tile_x + tile_y * 32;
    uint8_t tile = ppu->vram[tile_address];
...
void get_sprite(ppu_t *ppu, lcd_t *lcd, int actual_screen_x, int actual_screen_y, int *palette_index, int *color,
    ...
    *sprite_found = 0;
    ...
    for (int s = 0; s < 40; s++) {
        ...
        if (screen_y >= sprite_y && screen_y < sprite_y + sprite_height && sprites_on_ly_count < 10) {
        ...
        if (screen_x >= possible_sprite_x && screen_x < possible_sprite_x + 8) {
            if (!*sprite_found) {
                ...
                *sprite_found = 1;
                ...
                int replace_sprite =
                        possible_sprite_x > sprite_x || (possible_sprite_x == sprite_x && sprites_on_ly[s] < sprite);
                if (replace_sprite) {
                    sprite = sprites_on_ly[s];
                    sprite_x = possible_sprite_x;
                    *sprite_found = 1;
    ...
    if (*sprite_found) {
        ...
        *palette_index = (low_value | high_value << 1) & 0b11;
        *color =
                ((palette ? ppu->register_obp1 : ppu->register_obp0) & (0b11 << *palette_index * 2)) >> *palette_index *
                2;
    }
}
...
void draw_scanline(ppu_t *ppu, lcd_t *lcd) {
    ...
    for (int x = 0; x < FB_WIDTH; x++) {
        ...
        if (window_enabled && x + 7 >= ppu->register_wx && lcd->register_ly >= ppu->register_wy) {
        ...
        get_sprite(ppu, lcd, x, lcd->register_ly, &sprite_palette_index, &sprite_color, &sprite_found,
                   &sprite_priority);
...
int next_state(ppu_mode_e mode, int dots, uint8_t ly, uint8_t stat, lcd_t *lcd, ppu_t *ppu,
               interrupt_controller_t *interrupt_controller, ppu_mode_e *next_mode, int *next_dots, uint8_t *next_ly) {
    switch (mode) {
...
void ppu_tick(ppu_t *ppu, lcd_t *lcd, bus_t *bus, interrupt_controller_t *interrupt_controller,
              int machine_cycles_elapsed) {
    ppu->pending_dots += machine_cycles_elapsed * 4;

    ppu_mode_e mode = lcd->register_stat & STAT_PPU_MODE;

    while (next_state(mode, ppu->pending_dots, lcd->register_ly, lcd->register_stat, lcd, ppu, interrupt_controller,
                      &mode,
                      &ppu->pending_dots, &lcd->register_ly)) {
    }

    if ((lcd->register_stat & STAT_LYC_EQUALS_LY) && lcd->register_ly == lcd->register_lyc) {
        interrupt_controller_set_lcd_flag(interrupt_controller);
    }
